...
def depthFirstSearch(problem):
    ...
    gameMap = util.Stack()
    gameMap.push((problem.getStartState(), [], []))

    while not(gameMap.isEmpty()):
        nodePosition, actions, visited = gameMap.pop()

        if problem.isGoalState(nodePosition):
            return actions

        for neighbour, move, cost in problem.getSuccessors(nodePosition):
            if neighbour not in visited:
                gameMap.push((neighbour, actions+[move] , visited+[nodePosition]))
    ...
    while not gameMap.isEmpty():
        ...
        if nodePosition not in visited:
            ...
            for neighbour, move, neighbourCost in problem.getSuccessors(nodePosition):
                gameMap.push((neighbour, actions+[move], pathCost + [neighbourCost]))
